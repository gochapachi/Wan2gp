...
def load_models(model_type, override_profile = -1, output_type="video", config_id = None, runtime_model_type=None, track_as_main=True, gen=None, loading_callback=None, **model_kwargs):
    ...
    with model_unload_guard(), offload.loading_context(loading_callback, loading_model_ids):
        ...
        if hasattr(wan_model, "set_offload_handoff"):
            wan_model.set_offload_handoff(offloadobj, offloadobj)
    if len(args.gpu) > 0:
...
def n8n_generate_api(prompt, model_type="Wan2.1-T2V-1.3B", resolution="832x480", video_length=81, num_inference_steps=20, state=None,
    ...
    # 4. Prepare parameters for generate_media
    fps = 16 
    ...
    # Convert image path/URL strings to PIL Images (generate_media expects PIL objects)
    ...
    def _is_pil(v):
        try:
            ...
            if isinstance(_PILImage.Image, type) and isinstance(v, _PILImage.Image):
                return True
        except Exception:
            pass
        return hasattr(v, 'size') and hasattr(v, 'convert')
    ...
    with n8n_generate_lock:
      try:
        gen_params = {
            ...
            "override_profile": -1,
            "override_attention": "",
            "attention_sparsity": ui_defaults.get("attention_sparsity", 0.0),
            "temperature": 1.0,
            ...
            "mode": "generate",
            "plugin_data": {},
            "spatial_upsampler_parameters": spatial_upsampler_parameters,
        }
        import inspect
        sig_params = inspect.signature(generate_media).parameters
        has_var_keyword = any(p.kind == inspect.Parameter.VAR_KEYWORD for p in sig_params.values())
        if has_var_keyword:
            filtered_params = gen_params
        else:
...
if __name__ == "__main__":
    ...
    def mount_n8n_and_outputs(app):
        from fastapi.staticfiles import StaticFiles
        try:
            app.mount("/outputs", StaticFiles(directory=outputs_dir), name="outputs")
            print("[n8n API] Successfully mounted /outputs")
        except Exception as e:
    ...
    launch_gradio(demo, _deepy_hybrid, args,
        ...
        share=args.share,
        allowed_paths=list({save_path, image_save_path, audio_save_path, "icons", os.getcwd(), outputs_dir}),
    )
